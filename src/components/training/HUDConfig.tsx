...
import { Upload, Save, Trash2, LayoutGrid, Plus, X } from 'lucide-react';
import { cn } from '../../lib/utils';
import type { HUDProfile, HUDLine, HUDStat } from '../../types';

const LS_HUD_KEY = 'pokercoach_hud_profile';
...
function loadStoredProfile(): HUDProfile | null {
  const raw = localStorage.getItem(LS_HUD_KEY);
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { return null; }
}

function saveProfile(profile: HUDProfile) {
  localStorage.setItem(LS_HUD_KEY, JSON.stringify(profile));
}

export function HUDConfig() {
  const [profile, setProfile] = useState<HUDProfile>(() => {
  ...
  const handleJsonImport = () => {
    try {
      ...
      if (data.lines && Array.isArray(data.lines)) {
        ...
        setJsonInput('');
        alert('HUD importado correctamente');
      } else {
